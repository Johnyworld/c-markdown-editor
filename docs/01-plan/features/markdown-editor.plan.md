...
- [x] 기본 마크다운 문법 지원 (헤딩, 굵기, 기울임, 목록, 코드블록, 링크, 이미지, 표, 인용) — **외부 라이브러리 없이 직접 구현**
...
| FR-06 | 초기 페이지 진입 시 어떤 블록에도 포커스가 없는 상태로 시작                        | Medium   | Pending |
| FR-07 | 에디터 영역 바깥을 클릭하면 포커스된 블록이 모두 blur 처리됨                        | Medium   | Pending |
...
| FR-05 | 에디터 초기화 버튼 — 확인 다이얼로그 후 전체 삭제, 빈 블록 하나로 초기화          | Low      | Pending |
| FR-08 | 빈 문서(블록 없음 또는 빈 블록만 있는 상태)를 클릭하면 새 블록이 생성되어 입력 가능 | Medium   | Pending |
| FR-09 | 에디터 하단에 항상 클릭 가능한 여백 영역이 존재하며, 클릭 시 새 블록 추가 및 포커스 | Medium   | Pending |
...
| ---------------------------------------- | ------ | ---------- | -------------------------------------------------------------- |
| 직접 구현 파서의 엣지케이스 누락         | High   | Medium     | 지원 문법 범위를 명확히 정의하고 단계적으로 구현·검증          |
| 직접 구현 파서의 XSS 취약점              | High   | Medium     | DOMPurify로 출력 sanitize 처리 필수                            |
...
| Markdown Parser | marked.js / remark / **직접 구현**         | **직접 구현**            | 외부 의존성 제거, 커스텀 문법(_밑줄_, 코드 블록 예외) 완전 제어 |
...
| 0.2     | 2026-03-13 | 마크다운 파서 직접 구현으로 변경 (marked.js 제거) | gimjaehwan |
| 0.3     | 2026-03-13 | FR-06/07 추가 — 초기 포커스 없음, 에디터 외부 클릭 시 blur | gimjaehwan |
| 0.4     | 2026-03-13 | FR-05 수정, FR-08/09 추가 — 초기화 후 빈 블록, 빈 문서 클릭, 하단 여백 클릭 | gimjaehwan |
